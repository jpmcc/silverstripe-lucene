...
class ZendSearchLuceneWrapper {
     ...
     * @static
     */
    private static $createIndexCallbacks = [];
        ...
        foreach ($extendedClasses as $className) {
            ...
            foreach ($objects as $object) {
                ...
                if (!array_key_exists($object->ClassName . ' ' . $object->ID, $indexed)) {
                    $indexed[$object->ClassName . ' ' . $object->ID] = [
                        $object->ClassName,
                        $object->ID
                    ];
                }
    ...
    protected static function getChildren($object, $indexed = []) {
        foreach ($object->Children() as $child) {
            if (!array_key_exists($child->ClassName . ' ' . $child->ID, $indexed)) {
                $indexed[$child->ClassName . ' ' . $child->ID] = [
                    $child->ClassName,
                    $child->ID
                ];
            }
